...
rectangular column cross-sections**. Anything outside that scope produces an explicit warning; the
tool does not silently extrapolate, but it also does not refuse to compute — the judgement remains
with the engineer.
...
The experimental database, extraction records, exclusion log, data dictionary, and full analysis
code are published separately, with their own persistent identifier:
**[10.5281/zenodo.21872724](https://doi.org/10.5281/zenodo.21872724)**.

Use the Zenodo record for anything involving the underlying data; use this repository to run the
calculator.

## Running it

Open `index.html` in a browser, either from a local copy or through GitHub Pages:

**https://abdullahturer.github.io/punching-openings-tool/**

There is no build step, no server, no package installation, and no network request at runtime.
The calculator works offline and directly from a `file://` path.

## Figure mode

Append `?print=1` to the calculator URL to remove the fixed height and scrolling from the
calculation-chain and warnings panels, so that the whole chain and every warning are visible in a
single screenshot.

For example:

```text
https://abdullahturer.github.io/punching-openings-tool/?print=1
```

Add `&nocharts=1` to hide the design-chart region and the footer as well:

```text
https://abdullahturer.github.io/punching-openings-tool/?print=1&nocharts=1
```

The charts are published separately as vector figures, and a screenshot of them would only be a
lower-resolution duplicate.

`#print` and `#print-nocharts` work too, for environments that drop the query string from a
`file://` URL.

Normal use is unaffected: without these parameters, the panels remain scrollable so that the four
regions fit on one screen. Printing the page keeps the charts deliberately; only the dedicated
screenshot mode hides them.
...
raises the break-even warning. The full recipe is provided in
`docs/makale/00_sekil_tablo_listesi.md`.
...
Licensing is to be confirmed before the final release. The intended licences are:
